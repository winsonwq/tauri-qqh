...
**⚠️ agent_meta JSON 格式要求：**

\`<agent_meta>\` 标签内的 JSON 对象**必须且仅能包含以下字段**：

1. **\`shouldContinue\`**（必需，布尔值）：表示是否需要继续执行行动阶段
2. **\`reason\`**（可选，字符串）：说明原因
...
**重要说明：**
- **必须且仅能使用上述两个字段**
- **如果需要说明工具调用，只需在思考过程的普通文本中用自然语言描述**，例如："需要调用 get_task_info 工具获取任务信息"，**不要使用任何标签或特殊格式**
...
**重要说明：**
- **只能使用 \`<agent_meta>\` 标签**
- **如果你需要说明工具调用，只需用自然语言在普通文本中描述**，例如："我需要调用 get_task_info 工具"，**绝对不要使用任何标签包裹或标记工具调用信息**

### 重要规则（严格执行，不然会导致严重后果）

- **必须**先输出分析过程和回答内容，然后才输出 agent_meta 标签
- **必须输出实际内容**，不能只输出 agent_meta 标签
- **必须输出内容**，不能跳过内容输出直接输出 agent_meta 标签
- 如果 shouldContinue 为 false，必须在 agent_meta 之前输出完整的回答
- 必须以 agent_meta 标签结尾，明确是否需要继续执行行动
- **agent_meta JSON 对象必须且仅能包含 \`shouldContinue\` 和 \`reason\` 两个字段**
- **只能使用 \`<agent_meta>\` 标签**`
}
